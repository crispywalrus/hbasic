...
class RelaxedBucketSpec extends org.specs2.SpecificationWithJUnit with ScalaCheck {
  ...
  def is = sequential ^
    "Given a relaxed bucket containing n tokens, when I try to consume m tokens" ^
    "where m <= n, then I should get m tokens" ! {
      val mn0 = for { m <- choose(0, sup); n <- choose(m, sup) } yield (m, n)
      Prop.forAll(mn0) {
        case (m, n) =>
          TokenBucket.maker.relaxed.withTokens(n).make.tryConsume(m) must_== (TokenBucket.maker.withTokens(n - m).make, m)
      }
    } ^
